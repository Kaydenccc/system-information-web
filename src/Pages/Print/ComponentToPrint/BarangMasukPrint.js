...
import { dataBarangMasuk } from '../../../dummy';

const BarangMasukPrint = React.forwardRef((_, ref) => {
  return (
    <article className=" h-full w-full" ref={ref}>
      ...
      <main className="flex justify-start overflow-x-auto flex-col items-start py-4 ">
        <h1 className="self-center uppercase text-xl font-bold">Laporan Barang Masuk</h1>
        ...
        <table className="border-collapse min-w-[975.5px] table-auto w-full ">
          <thead>
            <tr>
              ...
              <th className="text-center border px-2 pb-2 border-slate-300 dark:border-slate-500 text-gray-800 dark:text-slate-300 ">Kondisi</th>
              <th className="text-center px-2 border pb-2 border-slate-300 dark:border-slate-500 text-gray-800 dark:text-slate-300 ">Status</th>
          ...
          <tbody className="bg-slate-50 dark:bg-gray-800 client">
            {dataBarangMasuk.map((data, i) => (
              <tr key={i} className="cursor-pointer">
                ...
                <td className="text-center relative py-2 px-2 border border-slate-300 dark:border-slate-600">{data.kondisi}</td>
                <td className="text-center relative border px-2 border-slate-300 dark:border-slate-600">{data.status}</td>
