...
const PeminjamanPrint = React.forwardRef((props, ref) => {
  return (
    <article className=" h-full w-full" ref={ref}>
      ...
      <main className="flex justify-start overflow-x-auto flex-col items-start py-4 ">
        <h1 className="self-center uppercase text-xl font-bold">Laporan Peminjaman</h1>
        ...
        <table className="border-collapse min-w-[975.5px] table-auto w-full ">
          <thead>
            <tr>
              ...
              <th className="text-start border px-2 pb-2 border-slate-300 dark:border-slate-500 text-gray-800 dark:text-slate-300 ">Keterangan</th>
          ...
          <tbody className="bg-slate-50 dark:bg-gray-800 client">
            ...
            {dataPengajuan.map((data, i) => (
              <tr key={i} className="cursor-pointer">
                <td className="relative py-2 md:px-2 border border-slate-300 dark:border-slate-600">{i + 1}</td>
                <td className="text-start relative border px-2 py-2 border-slate-300 dark:border-slate-600">{data.status}</td>
                <td className="text-start relative py-2 px-2 border border-slate-300 dark:border-slate-600">{data.name}</td>
                <td className="text-start relative border px-2 border-slate-300 dark:border-slate-600">{data.keterangan}</td>
                <td className="text-start relative border px-2 border-slate-300 dark:border-slate-600">{data.tanggal}</td>
...
export default PeminjamanPrint;
