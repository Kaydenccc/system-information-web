...
import { dataPengadaan } from '../../../dummy';

const PengadaanPrint = React.forwardRef((props, ref) => {
  return (
    <article className=" h-full w-full" ref={ref}>
      ...
      <main className="flex justify-start overflow-x-auto flex-col items-start py-4 ">
        <h1 className="self-center uppercase text-xl font-bold">Laporan Pengadaan</h1>
        ...
        <table className="border-collapse min-w-[975.5px] table-auto w-full ">
          <thead>
            <tr>
              ...
              <th className="text-center border pb-2 border-slate-300 dark:border-slate-500 text-gray-800 dark:text-slate-300 ">Jumlah</th>
              <th className="text-start border px-2 pb-2 border-slate-300 dark:border-slate-500 text-gray-800 dark:text-slate-300 ">Pengaju</th>
          ...
          <tbody className="bg-slate-50 dark:bg-gray-800 client">
            {dataPengadaan.map((data, i) => (
              <tr key={i} className="cursor-pointer">
                ...
                <td className="text-center relative py-2  border border-slate-300 dark:border-slate-600">{data.jumlah}</td>
                <td className="text-start relative py-2 px-2 border border-slate-300 dark:border-slate-600">{data.pengaju}</td>
                ...
                <td className="text-start relative border px-2 py-2 border-slate-300 dark:border-slate-600">{data.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
