...
import { dataPengembalian } from '../../dummy';
    ...
    <table className="border-collapse md:border-separate table-fixed md:table-auto w-full md:w-full">
      <thead>
        <tr>
          ...
          <th className="text-start border-b border-slate-300 dark:border-slate-500 pl-6 pb-2 text-gray-800 dark:text-slate-300">Peminjam</th>
          ...
          <th className="text-start border-b border-slate-300 dark:border-slate-500 pb-2 pl-6 text-gray-800 dark:text-slate-300 hidden md:table-cell">kembali</th>
          <th className="text-start border-b border-slate-300 dark:border-slate-500 pb-2 pl-6 text-gray-800 dark:text-slate-300 hidden md:table-cell">Rusak</th>
          <th className="text-start border-b border-slate-300 dark:border-slate-500 pb-2 pl-6 text-gray-800 dark:text-slate-300 hidden md:table-cell">Hilang</th>
          ...
          <th className="text-end md:text-start border-b md:pl-6 border-slate-300 dark:border-slate-500 pb-2  pr-6 md:pr-0 text-gray-800 dark:text-slate-300">Tanggal</th>
      ...
      <tbody className="bg-slate-50 dark:bg-gray-800 client">
        {dataPengembalian.map((data, i) => (
          <tr key={i} className="cursor-pointer">
            <td className="border-b relative py-2 md:pl-6 hidden md:table-cell border-slate-300 dark:border-slate-600">{i + 1}</td>
            <td className="text-start border-b relative border-slate-300 dark:border-slate-600 pl-6 py-2">{data.peminjam}</td>
            <td className="border-b relative py-2 pl-6  border-slate-300 dark:border-slate-600">{data.barang}</td>
            <td className="border-b relative py-2 pl-6  border-slate-300 dark:border-slate-600 hidden md:table-cell">{data.kembali}</td>
            <td className="border-b relative py-2 pl-6  border-slate-300 dark:border-slate-600 hidden md:table-cell">{data.rusak}</td>
            <td className="border-b relative py-2 pl-6  border-slate-300 dark:border-slate-600 hidden md:table-cell">{data.hilang}</td>
            <td className={`text-center border-b relative border-slate-300 dark:border-slate-600 !w-fit !h-fit py-2 `}>
              <span className={`px-[10px] py-[2px] font-medium rounded-full ${data.status === 'Proses' && 'bg-yellow-300 text-yellow-700'} ${data.status === 'Selesai' && 'bg-green-300 text-green-700'}`}>{data.status}</span>
            </td>
            <td className="text-end md:text-start border-b relative border-slate-300 dark:border-slate-600 md:pl-6 pr-6 md:pr-0 py-2">{data.tanggal}</td>
