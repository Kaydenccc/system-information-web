...
import { dataSupplier } from '../../dummy';
...
const TableSuplier = () => {
  useEffect(() => {
    ...
    const getClientPositinX = (e) => {
      ...
      console.log('this is table suplier', y);
    ...
    <table className="border-collapse md:border-separate table-fixed md:table-auto w-full md:w-full">
      <thead>
        <tr>
          ...
          <th className="text-start border-b border-slate-300 dark:border-slate-500 pb-2 pl-6 text-gray-800 dark:text-slate-300">Asal</th>
          <th className="text-start border-b border-slate-300 dark:border-slate-500 pb-2 pl-6 text-gray-800 dark:text-slate-300 hidden md:table-cell">Alamat</th>
          <th className="text-start border-b border-slate-300 dark:border-slate-500 pb-2 pl-6 text-gray-800 dark:text-slate-300 ">No. HP</th>
      ...
      <tbody className="bg-slate-50 dark:bg-gray-800 client">
        {dataSupplier.map((data, i) => (
          <tr key={i} className="cursor-pointer">
            ...
            <td className="text-start border-b relative border-slate-300 dark:border-slate-600 pl-6 py-2">{data.name}</td>
            <td className="border-b relative py-2 pl-6  border-slate-300 dark:border-slate-600">{data.asal}</td>
            <td className="border-b relative py-2 pl-6  border-slate-300 dark:border-slate-600 hidden md:table-cell">{data.alamat}</td>
            <td className="border-b relative py-2 pl-6  border-slate-300 dark:border-slate-600  ">{data.noHp}</td>
